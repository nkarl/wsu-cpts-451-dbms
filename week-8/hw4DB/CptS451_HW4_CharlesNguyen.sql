...
FROM
    Student S,
    (SELECT DISTINCT
        PR.sid, P.courseno, PR.precourseno, PR.grade
        FROM    Prereq P,
                (SELECT DISTINCT
                    enroll.sid, prereq.precourseno, enroll.grade
                    FROM enroll, prereq
...
WHERE
    S.sid=LOW.sid
ORDER BY
    S.sname, LOW.courseno
;

-- 8
SELECT DISTINCT
    *,
    passing*100/total AS rate
...
WHERE
    Passed.courseno=Rolls.courseno
    -- AND Rolls.courseno LIKE "CptS%"
...
SELECT *
FROM (SELECT
        C.courseno, COUNT(P.precourseno) AS pcount
